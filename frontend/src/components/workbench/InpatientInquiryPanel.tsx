 ...
 *   - 入院诊断、入院病情（stable/critical/serious）
 *   - 体温/血压/心率/体重等体征
 *
 * 与门诊 InquiryPanel 的差异：
 *   - 含入院诊断字段（admission_diagnosis）
 *   - 含病情等级滑块（condition_level: 1-5）
 *   - 保存时调用 POST /encounters/{id}/inquiry（与门诊共用同一接口，但 payload 字段不同）
 *
 * 自动保存：
 *   - 字段变化时实时同步到 workbenchStore.inquiry
 *   - 点击「保存问诊」时将 inquiry 持久化到服务端
 *   - 完成后标记 CheckOutlined 2s 后恢复
 */
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '12px 16px' }}>
        ...
        >
          ...
          >
            <TextArea
              rows={3}
              placeholder="心脑血管、肺、肝、肾、内分泌系统疾病史；手术史、外伤史、传染病史、输血史、预防接种史、用药史"
              style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
            />
          </Form.Item>
          ...
          <Form.Item
            ...
            label={
              <span style={labelStyle}>
                食物/药物过敏史 <span style={{ color: '#ef4444' }}>*</span>
              </span>
            }
          >
            <Input
              placeholder="如：青霉素过敏 / 否认食物、药物过敏史"
          ...
          >
            <TextArea
              rows={2}
              placeholder="出生地及长期居留地、生活习惯及嗜好、职业与工作条件、毒物/粉尘/放射性物质接触史、冶游史"
              style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
            />
          </Form.Item>
          ...
          >
            <TextArea
              rows={2}
              placeholder="婚姻状况、结婚年龄、配偶及子女健康状况"
              style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
            />
          </Form.Item>
          ...
          {patientGender === 'female' && (
            <Form.Item
              ...
              label={
                <span style={labelStyle}>
                  月经史 <span style={{ color: '#ef4444' }}>*</span>
                </span>
              }
            >
              <TextArea
                rows={2}
                placeholder="初潮年龄、行经期天数、间隔天数、末次月经时间（或闭经年龄）、月经量、痛经及生育情况"
                style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
              />
            </Form.Item>
          )}
          ...
          >
            <TextArea
              rows={2}
              placeholder="父母、兄弟、姐妹健康状况，有无遗传倾向疾病"
              style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
            />
          </Form.Item>
          ...
          <Form.Item
            ...
            label={
              <span style={labelStyle}>
                当前用药 <span style={{ color: '#ef4444', fontSize: 10 }}>（缺失扣1分）</span>
              </span>
            }
          >
            <TextArea
              rows={2}
              placeholder="入院前正在使用的药物（药名、剂量、用法），无则填写：无"
              style={{ borderRadius: 6, fontSize: 13, resize: 'none' }}
            />
          </Form.Item>
          ...
          <Form.Item
            ...
            label={
              <span style={labelStyle}>
                宗教信仰{' '}
                <span style={{ color: '#ef4444', fontSize: 10 }}>（影响饮食/用药，缺失扣1分）</span>
              </span>
            }
          >
          ...
          >
            <Select placeholder="请选择营养风险" style={{ width: '100%' }} size="small" allowClear>
              ...
              <Select.Option value="存在营养风险，需营养支持">
                存在营养风险，需营养支持
              </Select.Option>
              <Select.Option value="营养不良，需积极营养干预">
                营养不良，需积极营养干预
              </Select.Option>
            </Select>
          </Form.Item>
      ...
      >
        <Button
          ...
          style={{
            ...
            height: 36,
            fontWeight: 600,
            background: isDirty ? 'linear-gradient(135deg, #0369a1, #0ea5e9)' : '#86efac',
            border: 'none',
