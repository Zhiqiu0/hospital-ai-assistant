      ...
      content: (
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)' }}>
            {/* 文案与实际行为对齐（2026-09-02 出院流程实测）：原文写「将从病区
                列表移除」，但实际行为是**留在列表里并标注「待补出院记录」**，
                直到出院记录签发为止——这正是规范要求的「出院后 24 小时内完成
                出院记录」的落地方式。文案说会移除，医生按此预期办完出院就走，
                不知道还欠一份出院记录，等于把这条时效提醒废掉。 */}
            出院后患者会留在病区列表并标注「待补出院记录」，直到出院记录签发；
            已签发病历可在「历史病历」中查阅。
          </div>
        </div>
      ),
  ...
  return (
    ...
    >
      ...
      <Space size={4}>
        ...
        {/* 取消接诊：仅有进行中接诊时显示；与"办理出院"区分语义——出院是正常完成，取消是放弃 */}
        {currentEncounterId && onOpenCancel && (
          <Button
            ...
            style={{ color: '#94a3b8', fontSize: 12, borderRadius: 8 }}
          >
            取消接诊
          </Button>
        )}
