/**
 * recordSectionWriter 行级（子行）写入契约测试
 *
 * 2026-06-11 自 qcFieldMaps.test.ts（787 行超标）按被测模块拆分而来，测试内容零改动。
 * 本文件覆盖 writeSectionToRecord 的行级写入路径：
 * 中医四诊 / 住院专项评估 / 生命体征 whole_line / 同段多字段顺序无关性。
 *
...
// 注：原文件该 describe 里的 3 个"per-field 三态撤销"用例（被测函数是
// snapshotFieldState / restoreFieldState）已搬到 recordFieldState.test.ts。
