/**
 * 操作审计日志页（pages/admin/AuditLogsPage.tsx）
 *
 * 展示系统全量操作审计记录，调用 GET /admin/audit-logs（分页）：
 *   - 筛选：操作类型、操作人、时间范围、资源类型
 *   - 列：时间、操作人、操作类型（login/create/update/delete）、
 *         资源类型、资源 ID、IP 地址、结果（success/failure）
 *   - 失败操作标红显示
 *
 * 审计记录只读，不支持删除（合规要求保留全量日志）。
    ...
    <div>
      ...
      <Table
        ...
        pagination={{
          ...
          onChange: p => {
            setPage(p)
            loadLogs(p, keyword, actionFilter)
          },
          showTotal: t => `共 ${t} 条日志`,
