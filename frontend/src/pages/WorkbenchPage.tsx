...
export default function WorkbenchPage({ mode = 'outpatient' }: WorkbenchPageProps) {
  ...
  // 接诊场景响应式订阅：恢复或转住院后都应去住院工作台，不在门诊渲染住院表单。
  // 路由懒加载会延迟旧页面卸载，不能用定时器猜测导航完成后清空全局接诊。
  ...
  // 保留接诊、转诊正文及患者缓存；导航等待期间也不暴露错误场景的业务操作。
  if (activeVisitType === 'inpatient') return <Navigate to="/inpatient" replace />
