 ...
 * 2026-06-11 Round 5 拆分：
 *   - 一次录音的完整流程编排（麦克风/实时 ASR/MediaRecorder/卸载清理）
...
export function useVoiceInputCard({
  ...
  // 录音会话编排（麦克风 / 实时 ASR / MediaRecorder / 卸载清理）
  // → hooks/useVoiceDialogueSession.ts（2026-06-11 Round 5 拆分）
  const { recordingSupported, listening, uploadingAudio, startListening, stopListening } =
