...
export function useAutoSaveDraft({
  ...
  const performSave = async (payload: DraftPayload): Promise<boolean> => {
    ...
    try {
      ...
      // 永久性拒绝（2026-08-13 第五轮审计修复；2026-09-02 补 400）：
      //   403 = 病历已签发 / 无权写
      //   404 = 接诊或病历已不存在
      //   400 = 请求本身不合法（如文书类型与就诊类型不符、recorded_at 在未来）
      // 这些重试多少次都还是同样的结果，原先却和网络错误一样入失败队列——
      // 队列条目永远补发不成功，后续所有草稿都堵在它后面，医生的草稿保存链路
      // 被**永久毒化**且没有任何提示。直接清掉该条目并明确告知医生。
      //
      // 400 是 2026-09-02 补的：住院支线实测时给后端加了「文书类型必须与就诊
      // 类型相容」的守卫，存量脏数据（住院接诊挂着 outpatient 病历）立刻让
      // auto-save 每 5 秒 400 一次、还全部进重试队列——守卫本身是对的，但不配
      // 前端这一手就等于把毒化链路又造了一遍。
