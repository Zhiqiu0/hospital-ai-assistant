...
class PatientResponse(BaseModel):
    ...
    注意"普通医生只能看到自己接诊过的患者"这句原话已于 2026-08-13 放开读时
    失效（改为全院共享不拦读、靠审计留痕追责），2026-09-01 数据边界审计更正
    ——过时的安全断言比没有断言更危险，它让人以为这里还有一道并不存在的闸门。
    当前实际边界：医生 + 管理角色可读（core/authz.PATIENT_READ_ROLES），
    每次查阅写审计日志。
    """
