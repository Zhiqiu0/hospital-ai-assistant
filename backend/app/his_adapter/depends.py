...
提供 FastAPI 依赖 `require_his_enabled`，所有 HIS 对接路由（/his/* 推送、
回写、WS、叫号）必须挂这个依赖。开关由 `settings.his_adapter_enabled` 控制：
  - True  → 放行
  - False → 直接抛 503 ServiceUnavailable，前端能感知"HIS 对接未启用"

为什么需要这层保险丝：
  HIS 对接与 SaaS 共用 service 层代码，万一对接链路触发了某个共用 service
  的 race condition 或边界 bug，可能波及 SaaS 用户。开关让运维 5 秒切回
  纯 SaaS 状态：改环境变量 + 重启后端 → 全院对接功能下线，SaaS 0 影响。
...
async def require_his_enabled() -> None:
    ...
    使用方式（单路由挂或整个 router 注册时挂）：
        @router.post("/encounter/admit", dependencies=[Depends(require_his_enabled)])
        async def admit_push(...): ...
