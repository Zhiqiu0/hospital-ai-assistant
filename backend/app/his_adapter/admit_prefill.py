...
async def _prefill_from_push(
    ...
    for k in dropped:
        # 只记字段名不记数值（2026-09-23 日志审计修）：体征数值+接诊号可
        # 关联患者，属健康信息不进日志；「哪个字段超限被丢」足以定位
        vitals_data.pop(k)
        logger.warning(
            "his_admit.prefill_vitals: %s 超生理极限已丢弃 encounter=%s",
            k, encounter_id)
    ...
    if _sys is not None and _dia is not None and _sys <= _dia:
        # 同上：真实血压读数不进日志，「收缩≤舒张被丢弃」本身已说明原因
        vitals_data.pop("bp_systolic")
        vitals_data.pop("bp_diastolic")
        logger.warning(
            "his_admit.prefill_vitals: 血压收缩≤舒张（疑似错位）已丢弃 encounter=%s",
            encounter_id)
