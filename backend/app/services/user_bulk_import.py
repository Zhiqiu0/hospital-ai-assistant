...
async def bulk_import_doctors(
    ...
    # 兜底捕获包住**整个循环**（2026-08-13 第四轮审计修复）：
    # 上一版把 try 只包在 commit 上，而循环里每轮都有 db.flush()（拿 user.id 用），
    # 加上 select 触发的 autoflush——约束冲突在 flush 那一刻就抛了，会话立刻进入
    # PendingRollbackError，写在后面的 except 根本轮不到执行，照样 500 且整批丢失。
    # 同时补 DataError：字段超长（名单里粘进备注列时 codes[0] 会是超长串）抛的是它，
    # 不是 IntegrityError。
    try:
        for item in data.items:
            ...
            if exists is not None or username in batch_usernames:
                ...
                # 预检也要算出「会补挂哪些工号」（2026-08-13 第四轮审计修复）：
                # 原先补挂整段被 not dry_run 挡住，预检恒显示"该用户名已存在，未改动"，
                # 而补挂是本函数里唯一会改 HIS 工号归属的写操作——预检看不见它自己
                # 所把关的那次写入，等于 dry_run 的契约在这条路径上失效。
                # 更实的后果：预检与真跑结果不一致（真跑时补挂占用了工号，
                # 同批后面用同一工号的行会从 created 变成 code_conflict）。
                added: list[str] = []
                if exists is not None:
                    for code in codes:
                        ...
                        if taken is not None or code in batch_codes:
                            continue
                        if not data.dry_run:
                            db.add(DoctorCode(user_id=exists.id, code=code, note="补挂工号"))
                        added.append(code)
                        batch_codes.add(code)   # 预检同样占位，两条路径判定一致
                entry.status = "skipped_exists"
                entry.message = (
                    f"账号已存在（未改动），{'将' if data.dry_run else ''}补挂工号：{','.join(added)}"
                    if added else "该用户名已存在，未改动"
                )
