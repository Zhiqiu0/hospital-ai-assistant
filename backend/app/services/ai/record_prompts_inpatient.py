...
5. 病程类记录（日常病程/上级查房/出院记录/术前小结/手术记录/术后病程）里的
   **住院期间已发生事实**——是否已手术及术式、麻醉方式、当前伤口/敷料/渗出状态、
   已执行的用药与治疗、化验回报、知情同意签署、上级医师意见——只能来自医生录入，
   录入里没有就写 "[未填写，需补充]"。**不得**用"敷料干燥/伤口愈合良好/已予抗感染/
   术后恢复顺利/已完善术前检查/已签署知情同意书"等貌似合理的推测填充，也不得把
   入院时的体征照抄成"当日查体"（当日查体未录入时写占位符）。
   建议性内容不受此限：手术指征、预计风险及预防措施、下一步诊疗建议、观察要点
   可以基于录入的伤情正常给出（2026-08-25 黄金病例集实测：模型曾编造
   "臂丛麻醉/敷料干燥无渗液/已予抗感染"，与录入"出血活跃"直接矛盾）。"""
