...
def render_first_course_record(data: dict, **_extra) -> str:
    ...
    return _render_bracketed_sections(
        ...
        # 标题只留文书名（2026-09-01 住院支线实测修）：原文在文书名后还跟了一句
        # 「（书写时间：入院后__小时内完成）」——那是**空白模板上给医生看的书写
        # 要求**，不是病历内容；而且那个 __ 是硬编码字面量，从来没有被任何代码
        # 替换过。结果每一份首次病程记录的正文第二行都印着这句带空格线的提示，
        # 签发后原样进法定文书。时限提醒该待的地方是工作台的「文书时效」面板
        # （那里已经在显示「剩余 8.0 小时」），不是病历正文。
        title_line="首次病程记录",
    )
