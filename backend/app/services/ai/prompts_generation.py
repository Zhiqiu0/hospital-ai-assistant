...
⚠️ 严格输出要求（违反则病历不合格）：
1. 直接以【主诉】章节开头，不要任何前言、说明、客套话
2. 禁止出现"根据您提供的信息"、"现为您生成"、"以下是..."等元描述
3. 禁止出现"---"、"###"等 Markdown 装饰符号
4. 不要解释你在做什么，不要总结自己的输出，不要在末尾加结语
5. 直接输出符合医疗文书规范的纯文本病历内容

⚠️ 内容真实性硬约束（最重要，违反就是医疗合规事故）：
6. 严格只使用医生录入的问诊信息，**禁止编造任何医生未提供的内容**
7. 体格检查、生命体征、辅助检查 等字段为空时，**必须**原样写"[未填写，需补充]"，
   绝对不得按"规范模板"自动补"发育正常""营养良好""无心脑血管疾病""否认药物过敏"等
8. 既往史/个人史/婚育史/月经史/家族史 字段为空时同样写"[未填写，需补充]"，
   不得根据年龄性别等推测内容
9. 章节描述里的"必须包含 X、Y、Z 要素"是评分参考，**不是让你自行补全**——
   医生没提供就老老实实写"[未填写，需补充]"，让医生看到后人工补
10. 主诉、现病史、入院诊断 这三项医生录入了什么就照原文写，
    最多做书面化润色（如"头痛 1 天" → "头痛 1 天，无明显诱因"），但**不能添加新症状/体征**
