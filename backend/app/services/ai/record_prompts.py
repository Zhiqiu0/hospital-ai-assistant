...
1. 现病史/既往史/个人史/家族史/月经史/望诊/闻诊/其余阳性体征/复诊建议/注意事项等叙述性字段：
   ...
   而不是压缩成"否认"）。
2. 诊断/舌象/脉象/治则治法/处理意见（药名剂量用法）仍严格照抄，不改写；
   主诉按真实性约束 3a 只做用词规范化。
3. 药物与治疗方式**不得具体化**：医生说"开了药/吃了药"只能写"予药物治疗/服药治疗"，
   不得推断成"中药/西药/输液"或任何具体药名；说"做了治疗"只能写"予对症治疗"。
   （真实测试抓到过把"开了药"写成"予中药治疗"——这是编造）"""


# ─── 复诊书写规则（2026-08-20 对照濮氏真实复诊病历补） ─────────────────
#
# 前端复诊时一直把上次病历全文（previous_record）随请求下发，但 L3 结构化
# 改造后 prompt 没接这个字段——复诊生成拿不到上次内容，"病史同前"展不开、
# 稳定信息（既往史/诊断）无从保持一致。现接回：上次病历作只读参考注入，
# 并按医院真实复诊写法（"病史同前"+本次变化一句）约束各字段。
REVISIT_RULES_TEMPLATE = """━━━ 复诊书写规则（本次为复诊，严格遵守） ━━━
1. 现病史：以"病史同前"开头衔接，再写本次录入的变化（好转/无变化/加重）与
   诊治情况，一到三句即可，**不要整段复述上次病历的起病经过**
2. 延续性字段（诊断/月经史/病发时间等）由系统在复诊开始时自动带入医生录入，
   按"照抄医生录入"处理即可；医生有意清空的字段照常输出 [未填写，需补充]。
   上次病历里本身是 [未填写，需补充] 的行视为无内容，不得沿用占位符
3. 不得把上次病历里的症状当成本次新发症状写进现病史
4. 上次病历仅供参考衔接，其中的体征数值/舌脉**不得**照搬为本次结果——
   本次体征舌脉只能来自本次医生录入，没录就写 [未填写，需补充]

上次就诊病历（只读参考）：
{previous_record}"""


def _build_revisit_block(req: Any) -> str:
    """复诊且带上次病历时返回复诊规则块，否则空串（初诊 prompt 零变化）。"""
    if getattr(req, "is_first_visit", True):
        return ""
    prev = (getattr(req, "previous_record", None) or "").strip()
    if not prev:
        return ""
    # 截断护栏：上次病历超长时保留前 3000 字（正常门诊病历 <1500 字，
    # 超长多半是异常数据，别让它撑爆 prompt）
    return "\n\n" + REVISIT_RULES_TEMPLATE.format(previous_record=prev[:3000])
...
{OUTPATIENT_STYLE_RULES}{_build_revisit_block(req)}"""
