...
async def quick_supplement(
    ...
    prompt = SUPPLEMENT_PROMPT.format(
        ...
        current_content=req.current_content if req.current_content else "（空）",
...
async def quick_qc(
    ...
    try:
        ...
        result = await llm_client.chat_json_stream(
            messages,
            temperature=0,  # 强制0温度，避免每次质控结果随机抖动
            max_tokens=model_options["max_tokens"],
            model_name=model_options["model_name"],
        )
        usage = llm_client._last_usage
        task_id = await _log_task("qc",
                                   token_input=usage.prompt_tokens if usage else 0,
                                   token_output=usage.completion_tokens if usage else 0)
        # 标记 LLM 问题来源，去重（规则引擎已覆盖的字段不重复列出）
        llm_raw = result.get("issues", [])
        ...
        await _save_qc_issues(task_id, all_issues, encounter_id=req.encounter_id)

        # 评分只基于规则引擎（确定性）：处理完规则问题 = 真实100分
        rule_score, rule_level = _calc_grade_score(rule_issues + insurance_tagged)
        ...
        rule_score, rule_level = _calc_grade_score(rule_issues)
        all_fallback = rule_issues + [{**i, "source": "llm"} for i in insurance_issues]
        return {
            "issues": all_fallback,
