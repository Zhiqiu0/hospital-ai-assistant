...
- options中禁止出现"有"/"无"、"是"/"否"、"正常"/"异常"等互斥对立选项；选项均为正向具体描述，患者不勾选即代表阴性（例：询问过敏史→options应为["青霉素","磺胺类","阿司匹林","碘造影剂"]，而非["有过敏","无过敏"]）"""
...
async def quick_qc(
    ...
    # Run rule-based checks（住院病历传入所有专项字段；门诊传入中医字段）
    rule_issues = check_completeness({
        ...
        # 中医四诊 + 诊断 + 治疗字段
        "tcm_inspection": req.tcm_inspection or "",
        "tcm_auscultation": req.tcm_auscultation or "",
        "tongue_coating": req.tongue_coating or "",
        "pulse_condition": req.pulse_condition or "",
        "tcm_disease_diagnosis": req.tcm_disease_diagnosis or "",
        "tcm_syndrome_diagnosis": req.tcm_syndrome_diagnosis or "",
        "treatment_method": req.treatment_method or "",
        "treatment_plan": req.treatment_plan or "",
        "followup_advice": req.followup_advice or "",
        "onset_time": req.onset_time or "",
    }, is_inpatient=is_inpatient, record_text=req.content, is_first_visit=req.is_first_visit if req.is_first_visit is not None else True)
