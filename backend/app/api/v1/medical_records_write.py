# -*- coding: utf-8 -*-
"""病历·写路径路由（api/v1/medical_records_write.py，2026-09-19 读写两拆）。

纯机械搬移，零逻辑改动。本文件只放改数据的端点：auto-save-draft /
quick-save（签发）/ 创建 / 正文保存 / 导出审计留痕 / 空白草稿删除。
读路径见 medical_records_read.py；聚合挂载在 medical_records.py。
"""
# ── 第三方库 ──────────────────────────────────────────────────────────────────
...
async def log_record_export(
    ...
    **为什么这里不加接诊归属校验（2026-08-17 第 15 轮审计结论，别再报为漏洞）**：
    ...
    （患者信息对登录医生开放读、以 view_records 审计为防线，与 by-patient 同口径）。而三个
