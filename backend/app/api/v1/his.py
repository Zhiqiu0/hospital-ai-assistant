...
async def admit_push(request: Request) -> ApiEnvelope:
    ...
    try:
        ...
        # 入站被拒必须留痕（2026-09-02 可观测性专项）：联调期最常见的争议是
        # "我推了你没收到"，我方只回一个 ack 信封、日志无痕，就只能靠厂商那边
        # 的记录反推。这里连 body 都解不出来，能带的上下文只有 appId。
        logger.warning("his_admit.reject: 请求体非 UTF-8 app_id=%s",
    ...
    if not timestamp_fresh(timestamp, settings.his_sign_clock_skew_seconds):
        # 时钟不同步是联调头一天最常见的拦路虎，把双方时间戳都记下来
        logger.warning("his_admit.reject: 时间戳过期或非法 app_id=%s ts=%r 我方=%d",
                       app_id, timestamp[:20], int(time.time() * 1000))
        return err(40002, "时间戳过期或非法")
    if not app_id or app_id != settings.his_inbound_app_id:
        logger.warning("his_admit.reject: appId 无效 收到=%r", app_id[:40])
        return err(40003, "appId 无效")
    if not verify_sign(app_id, timestamp, nonce, body_raw, sign, settings.his_inbound_app_secret):
        # 只记签名前 8 位与报文长度：足以让双方比对，又不泄露完整签名
        logger.warning("his_admit.reject: 签名校验失败 app_id=%s sign=%s… body_len=%d",
                       app_id, (sign or "")[:8], len(body_raw))
    ...
    if not await redis_cache.claim_nonce(
        ...
        logger.warning("his_admit.reject: nonce 重复（疑似重放或重试）app_id=%s nonce=%s",
                       app_id, (nonce or "")[:16])
        return err(40006, "nonce 重复（疑似重放）")
    try:
        payload = AdmitPushRequest.model_validate_json(body_raw)
    except ValidationError as exc:
        # 记具体哪个字段不合格：厂商改一次推送格式要跑一轮联调，说清楚
        # 是哪个字段能省一轮。body 本身不入日志（含患者 PHI）。
        logger.warning("his_admit.reject: 参数校验失败 app_id=%s errors=%s",
                       app_id, [
    ...
    try:
        ...
        # 业务性拒收（工号未注册/挂多个账号等）——这类是**管理员要去处理**的，
        # 不是代码缺陷，用 warning 让它进 error.log 被看见
        logger.warning("his_admit.reject: visit_id=%s code=%s msg=%s",
                       payload.visit_id, exc.code, exc.message)
