...
def main() -> None:
    ...
    # 2.2 请求头表**不再加 X-Request-Id**（2026-08-15 撤回）。
    #
    # 这一行是我们自己在 v1.3 加进去的：它只存在于方案 A 的 HTTP 头里，而通道
    # 早在 2026-07-27 就已定为方案 B。于是整行的实际效果是——让厂商读五行字，
    # 读完结论是「本期贵方无需实现」。加它之前先降过一次优先级（A→C），
    # 但降级救不了一条本来就不该出现的条目：对读者是纯噪音。
    # 我方 HTTP 回落路径仍然会发这个头（writeback_sender._signed_headers），
    # 不写进规范也不构成冲突——2.5 已约定「忽略无法识别的字段」，
    # 真回落方案 A 时双方要重谈接口地址与凭证，届时再补文档不迟。
    ...
    # 这行是**唯一在首页指向附录 C 的路标**（变更说明 2026-08-15 已整体挪到文末）：
    # 一份规范该以标题开场，而不是以变更清单开场；但 A 类是要厂商动手前看的，
    # 所以必须在封面留一句把人引过去。数量一律由 CHANGES 算，不写死——
    # 写死的数字迟早和数据对不上，而这行恰恰是厂商唯一会读到的导航。
    _n_ab = len([c for c in CHANGES if c["prio"] in ("A", "B")])
    _n_a = len([c for c in CHANGES if c["prio"] == "A"])
    _edit_para(doc, "v1.2 修订：通道定稿",
               "v1.2 修订：通道定稿",
               "v1.3 修订：共 %d 处。其中需请您过目的 %d 处已整理成表，"
               "见文末《附录 C：本次修订说明》——A 类 %d 项建议在开发前先看；"
               "其余为口径写明，不改变接口本身、也不增加贵方工作量。"
               "正文中修订过的位置均以句首「▲」+ 淡底色标出。"
               "（v1.2 修订：通道定稿" % (len(CHANGES), _n_ab, _n_a),
               "C-扉页版本说明")

    # ══ 机械核对补漏（2026-08-15 第二轮：把「摘要说改了、正文没改」全清掉）══
    # 这轮不靠通读，靠逐项机械比对（字段表/枚举/错误码/数值/地址/交叉引用
    # 六类双向对比零差异），剩下的问题全集中在下面两类：
    #   ① 摘要声称改了某节，正文那节实际没动
    #   ② 旧内容只删了一半，同一张表里前后两行互相打架

    # 【B】第 4 章数据字典表：前两行是初版草案的遗留，与同表最后一行
    # 「无需提供任何字典」正面冲突；「诊断编码 ICD-10/GB95」还与 3.2
    # 「以名称为准、编码可能为空」及我方实际行为（从不下发 code/code_type）不符
    _edit_cell(T(11), "诊断编码", 1,
               "我方以诊断名称（name）为准，不下发 code / code_type；"
               "请贵方按名称匹配本院诊断库（编码能力后续迭代补充）",
               "B-第4章诊断编码行")
    _edit_cell(T(11), "科室编码", 1,
               "随接诊推送以 dept_code + dept_name 成对下发，无需预先交换字典",
               "B-第4章科室编码行")

    # 【C】2.4 错误码表 40006 行：摘要位置栏写的是「2.2 / 2.4」，
    # 但此前只改了 2.2，2.4 这行原样未动 → 拿摘要当 checklist 的人核不到
    _edit_cell(T(4), "40006", 1,
               "nonce 重复（疑似重放）。仅适用于方案 A 与 WebSocket 握手阶段；"
               "方案 B 的业务消息不做 nonce 去重，故不会出现在 WS 的 ack 中",
               "C-2.4错误码40006限定")


    # 【A】2.5：此前写「门诊/急诊 record_no 可空」，但我方实际恒下发（门诊恒为
    # "1"），且随附参考实现已明确写「不要为门诊做两段键特例」——两边正面打架，
    # 贵方照 2.5 做就会建出参考实现明确警告的那条特例分支
    _edit_para(
        doc, "幂等与建/更新：回写以（visit_id + record_type + record_no）",
        "门诊/急诊为单份病历（record_no 可空，visit_id+record_type 即唯一）；",
        "我方本期恒下发 record_no（门诊/急诊恒为 \"1\"），请一律按三段键定位，"
        "不必为门诊做两段键的特例分支；",
        "A-2.5恒下发record_no")

    # 【A】7.4 应答段补 ack 验签要求与失败后的表现。7.3 的「所有消息统一携带
    # sign」已涵盖 ack，但没写「验不过会怎样」——而那正是最难定位的故障：
    # 贵方以为自己回了 ack，我方这边表现为回写反复重发后断连
    _edit_para(
        doc, "应答（ack）：每条业务消息，接收方必须回一条 type=ack",
        "并额外携带 ack_msg_id 指向被应答消息的 msg_id。",
        "并额外携带 ack_msg_id 指向被应答消息的 msg_id。"
        "ack 与业务消息一样要按 7.3 签名（待签串末段为 ack 自己的 payload 原文）；"
        "我方对未通过验签的 ack 会直接丢弃且不回任何消息，"
        "表现为该条回写反复重发后断连——请对照附录 A.4 的示例。",
        "A-7.4ack验签说明")

    # ── 2026-08-15 第十轮审计（实现者视角 / 故障注入）─────────────────────────

    # 【A】7.3 sign 字段：写明签名是按**字节**算的，不是按 JSON 语义算的。
    # 这是从零写客户端时最可能卡住的一处：公式照抄没错，签名却一直对不上。
    # 实证：随附两份参考实现对同一份示例数据签出的结果就不同（Python 的标准
    # 序列化在冒号/逗号后带空格，JavaScript 不带），但两份都能通过我方验签。
    _edit_cell(
        T(13), "sign", 2,
        "签名，算法同 2.2，其中「请求body原文」= payload 的 JSON 序列化原文。"
        "关键：payload 只序列化一次——用于计算签名的那串字符，与本消息里 payload "
        "位置上的那串字符，必须逐字节相同（中文是否转义成 \\uXXXX、键的先后顺序、"
        "有无缩进空格，任一处不同签名即对不上，我方回 code=40001）。"
        "我方按贵方实际发出的报文原文验签，不会重新序列化。",
        "A-7.3签名字节口径")

    # 【C】7.3 msg_id 字段：必填性 + 取号规则 + ack 自身 msg_id 我方不使用。
    # 最后一条是放宽（我方已改实现）：ack 漏了自己的 msg_id 不再影响应答对应，
    # 否则又是一条「贵方回了 ack、我方当作没收到」的暗坑。
    _edit_cell(
        T(13), "msg_id", 2,
        "消息唯一 ID（发送方生成），用于 ack 对应与重发去重。业务消息必填，"
        "缺失我方回 code=40004 并说明原因；每条新消息都要取一个新号，"
        "只有同一条消息的超时重发才沿用原号（我方判重同时比对消息内容，"
        "内容不同的不会被当成重发）。ack 自身的 msg_id 我方不使用，"
        "漏填也不影响应答对应——对应只看 payload 里的 ack_msg_id。",
        "C-7.3msgId口径")

    # 【C】7.2：403 的四种成因并列。四者返回完全一样（403、无响应体、无 close
    # code），不列出来的话，贵方大概率一头扎进签名和密钥，而现场最常见的其实
    # 是诊室电脑时钟不准。
    _append_para_after(
        doc, "握手 nonce 为一次性",
        "握手返回 HTTP 403 有以下几种成因，返回内容完全一样、无从区分，"
        "请按此顺序自查：① appId 与我方分配的不一致；② 时间戳超出 ±5 分钟窗口"
        "（先看这台诊室电脑的系统时钟，长期不校时是最常见的一种）；"
        "③ 签名不符（见 7.3 的字节口径）；④ 握手 nonce 在窗口内重复（缓存了握手 URL）；"
        "⑤ 仅当双方约定启用了来源 IP 白名单时——出网 IP 不在名单内"
        "（2.2 提到的可选加强项，默认不启用；如需启用我方会先向贵方索取医院出口 IP）。",
        "C-7.2握手403成因清单")

    # 【C】7.4 应答段再补一句：信封不合法的 ack 我方也丢弃。与上面 msg_id 的
    # 放宽配套——把「我方到底怎么处理不合格的应答」一次说全，不留猜测空间。
    _edit_para(
        doc, "应答（ack）：每条业务消息，接收方必须回一条 type=ack",
        "表现为该条回写反复重发后断连——请对照附录 A.4 的示例。",
        "表现为该条回写反复重发后断连——请对照附录 A.4 的示例。"
        "同理，信封本身不合法（如不是合法 JSON）的 ack 我方也直接丢弃、不回消息；"
        "但 ack 漏填自身的 msg_id 不影响对应（见 7.3）。",
        "C-7.4坏ack处理")

    # 【C】3.1 改派边界：已签发的接诊不再改派。那份病历已用原医生名义签发并
    # 回写贵方，改派会让署名与接诊医生对不上。
    _edit_para(
        doc, "幂等与补推说明：幂等键为 hospital_code + visit_id",
        "若携带新的 doctor_code 会自动改派医生。",
        "若携带新的 doctor_code 会自动改派医生"
        "（该次接诊已签发/结束的除外——病历已用原医生的名义签发并回写贵方，"
        "此时改派会让病历署名与接诊医生对不上）。",
        "C-3.1改派边界")

    # ── 2026-08-15 第十一轮审计（厂商项目经理视角 + 端到端彩排实证）──────────

    # 【C】第 6 章第 5 步的「（待确认样式）」是我方内部待办词，出现在给厂商的
    # 验收条款里——厂商不知道要确认什么，也无从判断验收算不算通过。
    _edit_para(
        doc, "5. 验收：病历回写在医生界面",
        "病历回写在医生界面正确渲染（待确认样式）、可确认保存。",
        "病历回写后在 HIS 病历界面以草稿/待确认态正常显示，"
        "医生核对无误后可在 HIS 端确认保存，保存后即为正式病历。",
        "C-第6章验收条款")

    # 【C】第 6 章末句要求「先在测试环境跑通再上生产」——**我方没有测试环境**
    # （生产一套，服务器上只有 app-backend/db/redis）。这句让厂商以为可以先在
    # 我方测试环境试，实际不成立；更要紧的是端到端彩排已实证：联调产生的病历
    # 一旦签发就删不掉（产品有意不允许作废已签发病历），所以必须提前讲清
    # 「联调请用虚构患者」，否则真实患者信息会永久落进我方生产库。
    _edit_para(
        doc, "请贵方提供：测试环境的 HIS 客户端",
        "测试凭证由我方分配（方案 B 为单向一套，贵方无需为我方分配）。先在测试环境跑通再上生产。",
        "凭证由我方分配（方案 B 为单向一套，贵方无需为我方分配）。"
        "我方为单环境部署（即 wss://mediscribe.cn），联调与正式使用同一地址、"
        "同一套凭证，无需另配测试地址与测试凭证。"
        "因此请贵方联调时务必使用「虚构的测试患者」"
        "（姓名/身份证/手机号均可自拟），不要推送真实患者信息——"
        "已签发的病历按病历安全规则不可作废删除，会长期留在库中。"
        "联调期间产生的测试病历，由我方在医院正式启用前统一清理。",
        "C-第6章单环境与测试数据")

    # 【C】附录 B 补一条待确认：贵方客户端的开发语言。
    # 我方随附的是 Python 与 Node.js 两版参考实现；若贵方是 C# / Delphi，
    # 那两份只能当伪代码看，我方可针对性补该语言的注意事项
    # （例如 C# 的 ClientWebSocket 要显式选 Text 而不是 Binary）。
    _append_para_after(
        doc, "【待确认】测试环境与联调排期",
        "【待确认】贵方客户端的开发语言：我方随附 Python 与 Node.js 两版参考"
        "实现，若贵方用的是 C# / Java / Delphi 等，告知后我方可补该语言的"
        "对接注意事项（签名拼串与帧类型这两处最容易踩）。",
        "C-附录B客户端语言")
