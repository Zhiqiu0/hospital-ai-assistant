...
def main() -> None:
    ...
    # 【C】2.2 请求头表加 X-Request-Id。
    # 必须写明「仅方案 A」：这张表挂在「每个请求在 HTTP Header 携带以下公共
    # 参数」下面，读起来是双向通用的，而本期方案 B 的 WS 信封里根本没有这个
    # 字段（7.3 信封只有 7 个顶层字段）。不限定的话贵方会为它白做一遍。
    _add_row(T(2), ["X-Request-Id",
                    "请求级幂等键，仅用于方案 A 的病历回写方向（我方→贵方）。"
                    "同一份文书的所有重发（含超时重试与我方对账补投）该值恒定，"
                    "可据此做一层请求去重；不参与签名计算。"
                    "本期方案 B 的 WebSocket 信封无此字段，贵方无需实现——"
                    "请以 msg_id 与 2.5 业务幂等键判重"],
             "C-X-Request-Id（仅方案A）")
