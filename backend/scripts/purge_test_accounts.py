...
TEST_USERNAMES = [
    ...
    "qctest",
    # 2026-09-02 演练日建的两个测试账号（2026-09-17 终检预演的干跑抓漏）：
    # docbdrill 是越权测试医生、raddrill 是演练影像医师（radiologist）。两个
    # 都已停用，但停用账号带进生产同样是审计脏数据；raddrill 还会让"影像科
    # 是否已建真实账号"的核对失真（终检时差点被它冒充成真账号）。
    # 教训同上：演练建号必须当场补进本名单。
    "docbdrill",
    "raddrill",
]
