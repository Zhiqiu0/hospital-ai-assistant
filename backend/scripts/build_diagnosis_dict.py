...
def _pinyin_initial(name: str) -> str:
    ...
    已知限制（2026-08-31 姓名边界审计核实，**有意不改**）：
      pypinyin 对无拼音数据的生僻字（如 U+3402 㐂、U+24EC7 𤺋）返回空，
      该字被跳过导致整串左移——"疳𤺋"→"g"、"㐂病"→"b"。首字是生僻字的
      中医病名，医生按正常首字母输入必然 miss（startswith 语义下错位比
      丢字更麻烦）。
      不修的理由：无论保留什么占位符，医生都打不出生僻字的首字母，改索引
      只会引入新的错位风险；而汉字 contains 检索路径不受影响、始终可用
      （诊断字典的搜索同时走汉字与拼音两条路）。
      若将来要治本，正确方向是给这类词条人工补拼音字段，而不是猜。
    """
    out = []
    for ch in name:
        if ch.isascii():
