...
def main() -> None:
    ...
    n_ab = len([c for c in CHANGES if c["prio"] in ("A", "B")])
    n_c = len([c for c in CHANGES if c["prio"] == "C"])
    para_before(
        anchor,
    ...
    para_before(
        anchor,
        "其中 A 类 %d 项建议优先看——这几处若两边理解不一致，"
        "联调时会表现为「消息发出去了但对方没反应」这类不易定位的问题。"
        "另有 %d 处属口径写明（把原本没写清的地方说清楚，不改变接口本身，"
        "也不增加贵方工作量），不再单独罗列，已在正文对应位置以「▲」标出，"
        "通读时留意即可。"
        % (len([c for c in CHANGES if c["prio"] == "A"]), n_c),
        size=10.5, color=DARK, space_after=12, line=1.5,
    )

    # 只列 A、B 两类（2026-08-15 调整）：
    #   A = 你要对齐的，B = 你可以少做的，都需要厂商做动作。
    #   C 类 19 项是纯口径写明，正文里逐处已标 ▲，单独平铺在第一页只剩
    #   「我们有 19 处没写清楚」这一个观感——第一眼看到 30 行变更，
    #   厂商的第一反应是「这规范还没定下来」，而真正要他过目的只有 11 条。
    #   内容一条没少，只是从摘要页挪回它本来该在的地方（正文）。
    for prio in ("A", "B"):
