...
背景：`frontend/src/utils/recordExport.ts`（2026-09-19 两拆后标签表的家）的 RECORD_TYPE_LABEL 是手工维护的
...
_EXPORT_TS = (
    ...
    / "frontend" / "src" / "utils" / "recordExportShared.ts"
)


def _frontend_label_keys() -> dict[str, str]:
    """从 recordExport.ts 里抠出 RECORD_TYPE_LABEL（2026-09-19 两拆后标签表的家）的键值对。"""
    src = _EXPORT_TS.read_text(encoding="utf-8")
    m = re.search(
        r"RECORD_TYPE_LABEL:\s*Record<string,\s*string>\s*=\s*\{(.*?)\n\}",
        src, re.S,
    )
    assert m, "没能在 recordExportShared.ts 里定位 RECORD_TYPE_LABEL——它可能被改名或重构了"
...
def test_每个后端record_type都有中文打印标题():
    ...
    assert not missing, (
        f"病历类型 {sorted(missing)} 在 recordExport.ts（2026-09-19 两拆后标签表的家）的 RECORD_TYPE_LABEL 里没有"
...
def test_标题必须是中文():
    ...
    for key, label in _frontend_label_keys().items():
        assert re.search(r"[一-鿿]", label), (
            f"{key}（2026-09-19 两拆后标签表的家）的标题 {label!r} 不含中文——法定文书标题必须是规范中文名称"
        )
