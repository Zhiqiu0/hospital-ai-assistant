...
> **谁能开户**（2026-08-29 权限收紧）：批量开户与 HIS 工号绑定**仅院级管理员**
> （hospital_admin / super_admin）可用；科室管理员只能在本科室逐个建号、不能填工号、
> 不能建质控员/影像医师——按钮界面上都在，但权限不足会返回 403 并提示原因。

后台「用户管理 → 批量开户」粘贴人员名单，每行一位医生：

```
姓名, 工号1, 工号2, 工号3
汪来煜, 6069, 16029, 16039
```

同一位医生的多个 HIS 工号（本人门诊 / 本人住院 / 助理）写在同一行，
会挂到同一个账号——HIS 用其中任何一个工号推接诊都映射到本人，**病历署名恒为
账号的本人姓名**，助理工号推来的病历同样署本人名。第一个工号作为登录用户名。

先「预检」看清哪些会建、哪些跳过，再「确认导入」。全院使用统一初始密码，医生
首次登录被强制修改，改完之前服务端不放行任何患者/病历接口。
